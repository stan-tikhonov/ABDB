  ...
  navlistPanel(
    navbarMenu("Signatures of tissues",
      tabPanel("Brain",
        ...
        sliderInput(inputId = "brain_numsignifgenes",
        ...
        tableOutput(outputId = "brain_maintable")
...
server = function(input, output) {
  ...
  output$brain_maintable = renderTable({
    temp = (agingsignaturesapp[["Brain"]] %>% filter(presence_total >= input$brain_presence) %>% top_n(n = (-1 * input$brain_numsignifgenes), wt = adj_pval) %>% arrange(adj_pval))[, c("entrez", "genesymbol", "logFC", "pvalue", "adjusted_pvalue", "Brain_logFC", "Muscle_logFC", "Liver_logFC", "Mouse_logFC", "Human_logFC", "Rat_logFC", "All_logFC")]
    return(temp)
